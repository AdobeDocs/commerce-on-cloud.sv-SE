...
| 10 | validera-config | Filen `.magento.env.yaml` innehåller ett alternativ som inte har deklarerats i schemat, eller ett alternativ som konfigurerats med ett ogiltigt värde eller en ogiltig fas | Filen `./.magento.env.yaml` innehåller ogiltig konfiguration. Mer information finns i felloggen. |
...
| 17 | run-baler | Kommandot för att köra `Baler` för JavaScript-paketering misslyckades | Kontrollera miljövariabeln `SCD_USE_BALER` för att verifiera att Baler-modulen är konfigurerad och aktiverad för JS-paketering. Om du inte behöver balpressmodulen, ställ in `SCD_USE_BALER: false`. |
...
| 120 | compress-static-content | Det gick inte att komprimera statiskt innehåll | Mer information finns i `cloud.log`. |
